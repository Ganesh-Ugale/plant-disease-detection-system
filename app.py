...
def send_admin_email(name, email, plant_name, disease_name, description, img_path=None):
    """Send contact request email to admin via Resend API (works on Render free plan)."""
    try:
        ...
        <div style="max-width:600px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
            ...
            <div style="padding:30px;">
                <table style="width:100%;border-collapse:collapse;">
                    <tr><td style="padding:10px;background:#f9fafb;border-radius:8px;font-weight:bold;color:#065f46;width:140px;">👤 Name</td><td style="padding:10px;color:#1f2937;">{name}</td></tr>
                    <tr><td style="padding:10px;font-weight:bold;color:#065f46;">📧 Email</td><td style="padding:10px;"><a href="mailto:{email}">{email}</a></td></tr>
                    <tr><td style="padding:10px;background:#f9fafb;font-weight:bold;color:#065f46;">🌱 Plant Name</td><td style="padding:10px;font-weight:600;">{plant_name}</td></tr>
                    <tr><td style="padding:10px;font-weight:bold;color:#065f46;">🦠 Disease</td><td style="padding:10px;">{disease_name or 'Not specified'}</td></tr>
                    <tr><td style="padding:10px;background:#f9fafb;font-weight:bold;color:#065f46;vertical-align:top;">📝 Description</td><td style="padding:10px;line-height:1.6;">{description}</td></tr>
                </table>
                <div style="margin-top:25px;padding:15px;background:#f0fdf4;border-radius:8px;border-left:4px solid #10b981;">
                    <p style="margin:0;color:#065f46;font-size:13px;">⚡ Reply to this email to contact the user at <strong>{email}</strong></p>
                </div>
            </div>
        ...
        response = requests.post(
            ...
            json={
                "from": "Plant Disease System <onboarding@resend.dev>",
                "to": [ADMIN_EMAIL],
                "reply_to": email,
        ...
        if response.status_code == 200 or response.status_code == 201:
            print("✅ Admin email sent via Resend successfully!")
            return True
        else:
...
def submit_contact():
    try:
        ...
        thread = threading.Thread(
            ...
            args=(name, email, plant_name, disease_name, description, img_path),
            daemon=True
        )
        thread.start()
        ...
        return jsonify({
            'success': True,
            'message': 'Request submitted successfully! Admin has been notified.',
            'email_sent': True
        })
